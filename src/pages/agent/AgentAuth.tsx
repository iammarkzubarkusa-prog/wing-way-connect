    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          ...
          if (loggedUser) {
            ...
            if (data && data.is_approved) {
              ...
              toast({ title: "Pending approval", description: "Your account has not been approved yet", variant: "destructive" });
              await supabase.auth.signOut();
            } else {
              toast({ title: "Not an agent", description: "This account doesn't have agent access", variant: "destructive" });
              await supabase.auth.signOut();
            }
          }
        }
        ...
        if (error) {
          ...
          toast({ title: "Registration successful! 🎉", description: "Please verify your email. You can login after admin approval." });
        }
      }
    } catch {
      ...
      <div className="min-h-screen bg-hero-pattern flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="bg-card/95 backdrop-blur-xl rounded-3xl border border-border/50 p-8 shadow-2xl text-center">
            ...
            <p className="text-muted-foreground mb-6">Your agent account is awaiting admin approval. You'll be able to login once approved.</p>
        ...
        <div className="bg-card/95 backdrop-blur-xl rounded-3xl border border-border/50 p-8 shadow-2xl">
          ...
          <div className="mt-6 p-4 bg-muted/50 rounded-xl">
            <p className="text-xs text-center text-muted-foreground">
              {isLogin ? "After registration, you can login once admin approves your account" : "After registration, admin will approve your account"}
            </p>
          </div>
        </div>
